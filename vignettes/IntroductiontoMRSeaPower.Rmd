...
1) Fit model to baseline data.  
2) Decide on the kind of change you would like to impose.  
3) Simulate new pre and post impact observation data (noisy data). 
4) Update the baseline model to include impact term. 
5) Using simulated data, run power analysis. 

## 1) Baseline Model

If no baseline data is available, there are alternative methods (help files under development).

To use the functionality of this package, fit a `gamMRSea` model using the [`MRSea` package](https://lindesaysh.github.io/MRSea/).
...
```{r nysteddatplot, fig.cap='Figure showing the survey effort and proposed windfarm site for the Nysted Windfarm.', comment=FALSE, message=FALSE, fig.height=6, fig.width=8, echo=FALSE}
ggplot() + 
...
ggplot( NULL ) + geom_raster( data = rdf1 , aes( x , y , fill = pct.change ) ) +
  ...
  geom_path(data = nysted.bndwf, aes(x.pos, y.pos)) +
  geom_path(data = nysted.studybnd/1000, aes(x.pos, y.pos)) + 
  xlab("Easting") + ylab("Northing")
```
...
```{r, echo=FALSE}
nsim=300
empdistpower.ny<-getEmpDistribution(n.sim = nsim, simData=newdata.ny.imp, model = nysim_glm, data = impdata.ny, plot=FALSE, returnDist = TRUE, dots=FALSE)
...
```{r, echo=FALSE}
#summary.gamMRSea.power(powerout.nysted.re)
```
```{r powaukpowerplot, fig.cap='Figure showing how the power to detect change varies with the error rate chosen for the Forth and Tay redistribution analysis.  The first grey dashed line is at 1% and the second at 5%, traditionally values used as $p$-value cutoffs. The blue dashed lines indicate the error rate required to get a power of 80%.  The value is given in the title.', fig.height=6, fig.width=8, echo=FALSE}
powerPlot(powerout.nysted.re)

```


```{r powaukpredplot, fig.height=12, fig.width=8, fig.cap='Figure showing the mean (middle), lower 2.5% (top) and upper 97.5% (bottom) of predicted animal counts before (left) and after (right) the event.', echo=FALSE}
plot.preds(powerout.nysted.re, cellsize=c(1,1))
```


```{r powaukdiffsplot, fig.cap='Figure showing the mean (middle), lower 2.5% (left) and upper 97.5% (right) of estimated differences between before and after the event. (difference = post - pre)', fig.height=4, fig.width=8, echo=FALSE}
plot.diffs(powerout.nysted.re, cellsize=c(1, 1))
```

```{r powaukresigdiffre, fig.cap='Figure showing, for every grid cell, the proportion of simulations that showed a significant difference.', fig.height=6, fig.width=8, echo=FALSE}
