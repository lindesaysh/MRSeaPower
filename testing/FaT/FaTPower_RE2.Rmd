...
salsa1dlist<-list(fitnessMeasure='AIC', minKnots_1d = c(1,1), maxKnots_1d=c(5,5), startKnots_1d = c(1,1), degree=c(2,2), maxIterations=100, gaps=c(0,0))
...
```{r}
save(bestModel, file='FaT_model.RData')
```
...
```{r}
fatsim_glm<-update(bestModel1D, sim1~. + as.factor(eventphase), data=impdata)
fatsim_glm<-make.gamMRSea(fatsim_glm, panelid=1:nrow(impdata), splineParams=bestModel1D$splineParams, varshortnames=bestModel1D$varshortnames,gamMRSea=TRUE)
```
...
```{r}
par(mfrow=c(1,2))
quilt.plot(fat$x.pos, fat$y.pos, fitted(bestModel_int)[impdata$eventphase==0])
polymap(bnd.wf, add=T, lwd=3)
quilt.plot(fat$x.pos, fat$y.pos, fitted(bestModel_int)[impdata$eventphase==1])
polymap(bnd.wf, add=T, lwd=3)
```
...
nsim=10
powerout<-powerSimOverallChange(newdat, bestModel_int, empdistpower, nsim=nsim, powercoefid=length(coef(bestModel_int)), predictionGrid=predictdata, g2k=g2k, splineParams=bestModel_int$splineParams, sigdif=TRUE, n.boot=500, impact.loc=c(581877.4, 6234143))
```
