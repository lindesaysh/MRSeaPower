----
  title: "Forth and Tay: Redistribution simple model"
...
```{r fatinitglm}
init_glm<-glm(response ~ 1, data=fat, family=binomial)
```
...
salsa2dOutput<-runSALSA2D(init_glm, salsa2dlist, d2k=distMats$dataDist, k2k=distMats$knotDist, splineParams=NULL, tol=0, chooserad=F, panels=NULL, suppress.printout=TRUE)
```

```{r}
bestModel<-make.gamMRSea(salsa2dOutput$bestModel, panelid = fat$panel, splineParams = salsa2dOutput$splineParams, varshortnames = NULL, gamMRSea=TRUE)
...
nsim=500
#bnd.wf<-data.frame(x.pos=c(573582.6, 588287.9, 588888.2, 577484.0),y.pos=c(6245875, 6245278, 6221998, 6221998))
bnd.wf<-data.frame(x.pos=c(543289.6, 574317.5, 574892.1, 545588.0),y.pos=c(6253767, 6253097, 6237016, 6236681))
truebeta<-log(0.3)
impdata<-genRedistData(bestModel, data=fat, changecoef.link=truebeta, panels='panel', imppoly=bnd.wf)
...
```{r}
require(dplyr)
impdata$sim1<-newdat[,1]
group_by(impdata, impcells, eventphase)%>%
  summarise(tr=mean(truth),sim=mean(sim1) , num=n())
```
...
```{r}
par(mfrow=c(1,2))
quilt.plot(fat$x.pos, fat$y.pos, impdata$sim1[impdata$eventphase==0])
polymap(bnd.wf, add=T, lwd=3)
quilt.plot(fat$x.pos, fat$y.pos, impdata$sim1[impdata$eventphase==1])
polymap(bnd.wf, add=T, lwd=3)
```



Update the initial model to include the eventphase term and remove the spatial term.  The spatial term will be re-added as an interaction term via SALSA2D.

```{r}
fatsim_glm<-update(init_glm, sim1~. + as.factor(eventphase), data=impdata)
fatsim_glm<-make.gamMRSea(fatsim_glm, panelid=1:nrow(impdata), splineParams=NULL, varshortnames=NULL,gamMRSea=TRUE)
```

```{r}
salsa2dlist<-list(fitnessMeasure = 'BIC', knotgrid = knotgrid, knotdim = c(100, 100),  startKnots=10, minKnots=2, maxKnots=20, r_seq=r_seq, gap=0, interactionTerm='as.factor(eventphase)')

salsa2dOutputint<-runSALSA2D(fatsim_glm, salsa2dlist, d2k=rbind(distMats$dataDist,distMats$dataDist), k2k=distMats$knotDist, splineParams=NULL, tol=0, chooserad=F, panels=NULL, suppress.printout=FALSE)


bestModel_int<-make.gamMRSea(salsa2dOutputint$bestModel, panelid = 1:nrow(impdata), splineParams = salsa2dOutputint$splineParams, varshortnames = NULL, gamMRSea=TRUE)
...
empdistpower<-getRunsCritVals(n.sim = nsim, simData=newdat, 
                              model = bestModel_int, data = impdata, plot=TRUE, 
                              returnDist = TRUE, dots=FALSE)
```
...
nsim=10
powerout<-powerSimOverallChange(newdat, bestModel_int, empdistpower, nsim=nsim, powercoefid=length(coef(bestModel_int)), predictionGrid=predictdata, g2k=g2k, splineParams=bestModel_int$splineParams, sigdif=TRUE, n.boot=500, impact.loc=c(558385.2, 6245497))
```
...
null.output<-pval.coverage.null(newdat.ind = newdat, model = bestModel_int, nsim = 500, powercoefid = length(coef(bestModel_int)))
```

```{r}
summary(powerout, null.output, truebeta=truebeta)
```
