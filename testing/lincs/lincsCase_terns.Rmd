...
```{r}
par(mfrow=c(2,2))
quilt.plot(lincs$x.pos, lincs$y.pos, lincs$response, main='Original Data', ncol=40, nrow=40)
quilt.plot(lincs$x.pos, lincs$y.pos, fitted(bestModel), main='Fitted Values', ncol=40, nrow=40)
quilt.plot(lincs$x.pos, lincs$y.pos, newdatcor[,1], main='Simulated Data Example', ncol=40, nrow=40)
quilt.plot(lincs$x.pos, lincs$y.pos, apply(newdatcor, 1, mean), main='Mean Simulated Data', ncol=40, nrow=40)
...
predictdata<-rbind(data.frame(pred_grid[,1:3], month=10, area=1, eventphase=0), data.frame(pred_grid[,1:3], month=10, area=1, eventphase=1))
...
```{r aukpoweroc, cache=TRUE}
nsim=200
powerout.auk.oc<-powerSimPll(newdatcor.auk.imp, auksim_glm, empdistpower.auk, nsim=nsim, powercoefid=length(coef(auksim_glm)), predictionGrid=predictdata, g2k=g2k, splineParams=auksim_glm$splineParams, sigdif=TRUE, n.boot=200, nCores = 6)
```
...
```{r powauksummary}
summary(powerout.auk.oc, null.output.auk.oc, truebeta=log(truebeta))
```
...
```{r powauksigdiff}
plot.sigdiff(powerout.auk.oc, coordinates = predictdata[predictdata$eventphase==0,c('x.pos', 'y.pos')], tailed='two', error.rate = 0.05, gridcell.dim = c(1000,1000))
```

```{r powauksigdiffsid}
plot.sigdiff(powerout.auk.oc, coordinates = predictdata[predictdata$eventphase==0,c('x.pos', 'y.pos')], tailed='two', error.rate = 0.05, adjustment='sidak', gridcell.dim = c(1000,1000))
```

```{r powauksigdiffbonf}
plot.sigdiff(powerout.auk.oc, coordinates = predictdata[predictdata$eventphase==0,c('x.pos', 'y.pos')], tailed='two', error.rate = 0.05, adjustment='bonferroni', gridcell.dim = c(1000,1000))
```
...
<!-- ```{r} -->
<!-- summary(powerout.auk.oc25, null.output.auk.oc25, truebeta=log(truebeta)) -->
<!-- ``` -->


<!-- ```{r} -->
<!-- powerPlot(powerout.auk.oc25) -->

<!-- ``` -->

<!-- ```{r} -->
<!-- plot.preds(powerout.auk.oc25) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- plot.diffs(powerout.auk.oc25) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- plot.sigdiff(powerout.auk.oc25, coordinates = predictdata[predictdata$eventphase==0,c('x.pos', 'y.pos')], tailed='two', error.rate = 0.05, gridcell.dim = c(1000, 1000)) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- plot.sigdiff(powerout.auk.oc25, coordinates = predictdata[predictdata$eventphase==0,c('x.pos', 'y.pos')], tailed='two', error.rate = 0.05, adjustment='sidak', gridcell.dim = c(1000, 1000)) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- plot.sigdiff(powerout.auk.oc25, coordinates = predictdata[predictdata$eventphase==0,c('x.pos', 'y.pos')], tailed='two', error.rate = 0.05, adjustment='bonferroni', gridcell.dim = c(1000, 1000)) -->
<!-- ``` -->
