...
remove data points > 170 animals
```{r}
...
predictdata<-rbind(data.frame(preddata, month=10, area=1, eventphase=0), data.frame(preddata, month=10, area=1, eventphase=1))
...
nsim=400
powerout.auk.oc<-powerSimPll(newdata.auk.imp, auksim_glm, empdistpower.auk, nsim=nsim, powercoefid=length(coef(auksim_glm)), predictionGrid=predictdata, g2k=g2k, splineParams=auksim_glm$splineParams, sigdif=TRUE, n.boot=500, nCores = 6)
```
