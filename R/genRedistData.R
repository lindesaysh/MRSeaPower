...
genRedistData<-function(model, data, changecoef.link, panels=NULL, eventsite.bnd=NULL, noneventcells=NULL){

  if(is.null(eventsite.bnd) & is.null(noneventcells)){stop('One of noneventcells or eventsite.bnd must be specified')}
  ...
  if(is.null(noneventcells)){
    require(splancs)
    noneventcells<-ifelse(inout(data[,names(eventsite.bnd)], eventsite.bnd,quiet = T), 0, 1)
  }

  dat2<-rbind(data.frame(data, eventphase=0, panels=panels, nonevcells=0), data.frame(data,eventphase=1, panels=max(panels)+panels, nonevcells=noneventcells))
