runs.test<-function (y, plot.it = FALSE, alternative = c("two.sided", "positive.correlated",  "negative.correlated"), critvals=NULL) 
{
  ...
  if (alternative == "positive.correlated") {
    ...
    if(is.null(critvals)){
      ...
      p.value = length(which(critvals<(-abs(statistic)) | critvals>abs(statistic)))/length(critvals)
      alternative = "two.sided"
